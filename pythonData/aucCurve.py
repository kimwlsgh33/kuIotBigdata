...
def main():
    folder = "/Users/xxxx/devs/repos/kuIotBigdata/pythonData/data/"
    ...
    print(
        f"Precision(정밀도): {np.sum(true_pos) / (np.sum(true_pos) + np.sum(false_pos))}"
    )
    print(
        f"Recall(재현율): {np.sum(true_pos) / (np.sum(true_pos) + np.sum(false_neg))}"
    )

    fpr, tpr, thresholds = roc_curve(
        y, logit_reg.predict_proba(X)[:, 0], pos_label="default"
    )
    roc_df = pd.DataFrame(
        (
            {
    ...
    print(
        roc_auc_score(
            [1 if yi == "default" else 0 for yi in y], logit_reg.predict_proba(X)[:, 0]
        )
    )
